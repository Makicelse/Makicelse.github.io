...
- `git log --oneline -14`：查看最近 14 次的简略提交历史

- `git reset --soft/mixed/hard 目标版本哈希值`：回滚到哈希值（ID）指定的版本

  | 参数  | 表现                                                         | 是否清空暂存区 | 是否覆盖工作区 |
  | ----- | ------------------------------------------------------------ | -------------- | -------------- |
  | soft  | 代码回退到 commit 前的状态，不清空暂存区，不覆盖工作区       | 否             | 否             |
  | mixed | 代码回退，修改的文件变成了红色（未暂存状态）。工作区代码不变 | 是             | 否             |
  | hard  | 抹除所有改变的内容，彻底回退到提交前的状态。<br />保持 Untracked 文件（红色），回退提交（黑色或蓝色）和暂存区（绿色）中的文件或代码 | 是             | 是             |

  

### 其他

- `git rm --cached 文件路径`：删除新加入 .gitignore 的**已追踪文件**

- `git config --list`：检查本地仓库的**个人姓名、邮箱**
...
2. `type: ` 开头，总结提交的类型（如 `feat`、`fix`、`refactor` 等）。
3. `scope: ` 总结提交的范围（如 `backend`、`frontend` 等）。
4. `subject: ` 总结提交的内容。
```
