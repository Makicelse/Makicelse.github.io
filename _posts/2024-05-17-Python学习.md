...
- 参考文章：  
  ...
  **`ndarray`数组连接：**[NumPy: Concatenate arrays with np.concatenate, np.stack, etc.]([NumPy: Concatenate arrays with np.concatenate, np.stack, etc. | note.nkmk.me](https://note.nkmk.me/en/python-numpy-concatenate-stack-block/))    
